...
Admin_ID, COUNT( V_ID ) AS vcount FROM Vehicles   WHERE V_ID NOT IN (SELECT V_ID FROM Status_Development WHERE ('".$to."' >= From_date AND '".$from."' <= To_date)) and Type  = 'Cars' and City = '".$location."'
 GROUP BY Admin_ID,Vname");
   ...
   else
   {
      $result = mysql_query
("SELECT GROUP_CONCAT( V_ID )AS V_ID,Vname,Description,Mileage,Hourly_rate,Daily_rate,Location,img_name, 
Admin_ID, COUNT( V_ID ) AS vcount FROM Vehicles   WHERE V_ID NOT IN (SELECT V_ID FROM Status_Development WHERE ('".$to."' >= From_date AND '".$from."' <= To_date)) and Type  = 'Cars' GROUP BY Admin_ID,Vname");
   }
