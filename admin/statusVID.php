 ...
 $row = mysql_fetch_array($sql);
	  ...
	  if($row[0])
	   { 
             ...
             $date=$str. ' to ' .$str1;
      $result [0] =array('V_ID' => $row['V_ID'],'Vname' => $row['Vname'],'Status' => "Booked",'Details' => $date);
           ...
           {
 $sql = mysql_query
("SELECT V_ID,Vname FROM Vehicles WHERE  V_ID ='".$vid."' AND Admin_ID ='".$admin."' ");
       
 $row = mysql_fetch_array($sql);
  
              $result [0] =array('V_ID' => $row['V_ID'],'Vname' => $row['Vname'],'Status' => "Available",'Details' => "Not Booked");
